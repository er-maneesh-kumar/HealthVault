...
function Createpatient() {
  ...
  const [patient, setPatient] = useState({
    ...
    Family_History: '',
    avatar: null,
    // uid from localStorage user (stored as MongoDB _id, exposed as uid)
  ...
  const handleSubmit = async (e) => {
    ...
    if (!currentUser) {
      setError('You must be logged in to create a patient profile.');
      return;
    }
    ...
    try {
      ...
      // Use the logged-in user's MongoDB _id as the uid
      formData.append('firebaseUid', currentUser.uid);

      const token = localStorage.getItem('token');
      await axios.post(`${API_BASE}/backend/patients`, formData, {
        headers: {
      ...
      const msg = err.response?.data?.message || err.response?.data || 'An error occurred. Please try again.';
