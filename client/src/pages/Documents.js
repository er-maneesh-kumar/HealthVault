...
const Documents = () => {
  ...
  if (!currentUser) {
    return (
      <div className="documents-container" style={{ textAlign: 'center', minHeight: '60vh', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center' }}>
        <h2>Access Denied</h2>
        <p style={{ marginTop: '15px', fontSize: '1.2rem' }}>
          Please <Link to="/login" style={{ color: '#1464FF', fontWeight: 'bold', textDecoration: 'underline' }}>Login</Link> or <Link to="/signup" style={{ color: '#1464FF', fontWeight: 'bold', textDecoration: 'underline' }}>Sign Up</Link> first to view your medical documents.
        </p>
      </div>
    );
  }
